...
from benchmarks.network_loopback.harness.correctness import verify_behavioral_contract
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Verify S3 reactor and local-network contracts")
    parser.add_argument("--verify-only", action="store_true")
    parser.add_argument("--output-json", type=Path, default=BASE_DIR / "reports" / "network-loopback-correctness.json")
    ...
    if actual_s3_sha != EXPECTED_S3_SHA:
        print("NETWORK_LOOPBACK_BASELINE_PIN: FAIL", file=sys.stderr)
    ...
    report["runner"] = "tools/network_loopback_runner.py"
    args.output_json.parent.mkdir(parents=True, exist_ok=True)
    args.output_json.write_text(json.dumps(report, indent=2, sort_keys=True) + "\n", encoding="utf-8")

    if passed:
        print("NETWORK_LOOPBACK_BASELINE_PIN: PASS")
        print("REACTOR_NETWORK_CORRECTNESS: PASS")
        print("PERFORMANCE_STATUS: DEFERRED_UNTIL_NATIVE_REACTOR_ADAPTER_IS_BENCHMARKABLE")
        print(f"REPORT: {args.output_json}")
        return 0

    print("REACTOR_NETWORK_CORRECTNESS: FAIL", file=sys.stderr)
