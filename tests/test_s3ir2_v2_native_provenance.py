...
def _artifact_set(tmp_path: Path) -> tuple[Path, Path, Path, Path]:
    ...
    return candidate_source, binary, fixture, stream


def test_native_provenance_passes_only_when_real_files_match(tmp_path: Path) -> None:
    candidate_source, binary, fixture, stream = _artifact_set(tmp_path)
    report = validate(
...
def test_renderer_round_trips_into_validator(tmp_path: Path) -> None:
    candidate_source, binary, fixture, stream = _artifact_set(tmp_path)
    metadata = render(
    ...
    assert report["status"] == "PASS"
    assert metadata["candidate_binary_sha256"] == report["actual_files"]["candidate_binary"]["sha256"]
